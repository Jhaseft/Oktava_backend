  ...
  async sendPasswordResetEmail(to: string, code: string): Promise<void> {
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#0a0a0a;color:#fff;border-radius:16px;padding:40px 32px">
        ...
        <h2 style="font-size:20px;font-weight:700;margin:0 0 8px">Código de recuperación</h2>
        <p style="color:#a1a1aa;font-size:14px;margin:0 0 32px">
          Recibimos una solicitud para restablecer la contraseña de tu cuenta.<br/>
          Ingresa este código en la aplicación. Expira en <strong style="color:#fff">15 minutos</strong>.
        </p>
    ...
    try {
      await this.transporter.sendMail({ from, to, subject: `${code} es tu código de recuperación — Oktava`, html });
    } catch (err) {
      console.error('Mail send error (password reset):', err);
